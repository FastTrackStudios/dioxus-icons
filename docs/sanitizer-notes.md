...
- Rustdoc rejects self-closing SVG child tags under `rustdoc::invalid-html-tags`.
  Generated doc SVGs use explicit closing tags such as `<path></path>`.
...
- Widget and picker scripts avoid embedding literal SVG markup in JavaScript.
  Runtime SVG previews are read from data attributes or the JSON manifest.
