// @generated by dioxus-icons-codegen from Lucide v1.14.0 (56e49f12166312f04af4cfd862621c93cf583979). Do not edit.
// Lucide icons are licensed under ISC. See LICENSE-LUCIDE.

use dioxus::core::{Element, Template, TemplateNode};

use crate::IconProps;
use crate::vdom::{attr, circle, icon_element, icon_template, path, svg};

#[cfg_attr(any(doc, rust_analyzer), doc = include_str!("docs/calendar_cog.md"))]
#[cfg_attr(any(doc, rust_analyzer), doc = include_str!("../../../rustdoc-header.html"))]
#[allow(non_snake_case)]
pub fn CalendarCog(props: IconProps) -> Element {
    static TEMPLATE_ROOTS: &[TemplateNode] = &[svg(&[
        path(&[attr("d", "m15.228 16.852-.923-.383")]),
        path(&[attr("d", "m15.228 19.148-.923.383")]),
        path(&[attr("d", "M16 2v4")]),
        path(&[attr("d", "m16.47 14.305.382.923")]),
        path(&[attr("d", "m16.852 20.772-.383.924")]),
        path(&[attr("d", "m19.148 15.228.383-.923")]),
        path(&[attr("d", "m19.53 21.696-.382-.924")]),
        path(&[attr("d", "m20.772 16.852.924-.383")]),
        path(&[attr("d", "m20.772 19.148.924.383")]),
        path(&[attr(
            "d",
            "M21 10.592V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h6",
        )]),
        path(&[attr("d", "M3 10h18")]),
        path(&[attr("d", "M8 2v4")]),
        circle(&[attr("cx", "18"), attr("cy", "18"), attr("r", "3")]),
    ])];
    static TEMPLATE: Template = icon_template(TEMPLATE_ROOTS);

    icon_element(TEMPLATE, "0 0 24 24", props)
}
